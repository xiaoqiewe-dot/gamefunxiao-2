...
This repository currently has no commit history to infer a strict style. Use concise imperative commits, for example `Add flash mode room filtering` or `Fix menu click cancellation`. Pull requests should describe the change, list validation commands, mention affected config/resource files, and include screenshots or short clips for menu/UI changes.

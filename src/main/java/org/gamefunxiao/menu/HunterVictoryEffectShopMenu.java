...
public class HunterVictoryEffectShopMenu extends BaseMenu {

    private static final int[] EFFECT_SLOTS = {19, 20, 21, 22, 23, 24, 25, 31};

    public HunterVictoryEffectShopMenu(GameFunXiao plugin, Player player) {
        super(plugin, player, "§0§l⚔ 猎人胜利特效 ⚔", 54);
    }
    ...
    protected void setupItems() {
        ...
        inventory.setItem(4, createTitleItem(Material.FIREWORK_ROCKET,
                "§x§F§F§6§6§0§0⚔ §x§F§F§9§9§3§3猎§x§F§F§C§C§6§6人§x§F§F§F§F§9§9胜§x§C§C§F§F§9§9利§x§9§9§F§F§C§C特§x§6§6§F§F§F§F效",
                "§8· · · · · · · · · · · · · ·",
                "§f- §a余额: §e" + coins + " §6" + currency,
                "§f- §b当前选择: §d" + HunterVictoryEffect.byId(selected).getDisplayName(plugin),
                "§8· · · · · · · · · · · · · ·"));
    ...
    private ItemStack createEffectItem(HunterVictoryEffect effect, int coins, String currency, String selected) {
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        for (int i = 0; i < EFFECT_SLOTS.length && i < HunterVictoryEffect.values().length; i++) {
            if (slot == EFFECT_SLOTS[i]) {
                handleEffectClick(HunterVictoryEffect.values()[i]);
                return;
            }
        }
    ...
    private void handleEffectClick(HunterVictoryEffect effect) {
        ...
        if (plugin.getPlayerDataManager().hasVictoryEffect(player.getUniqueId(), effectId)) {
            plugin.getPlayerDataManager().setSelectedHunterVictoryEffect(player.getUniqueId(), effectId);
        ...
        plugin.getPlayerDataManager().unlockVictoryEffect(player.getUniqueId(), effectId);
        plugin.getPlayerDataManager().setSelectedHunterVictoryEffect(player.getUniqueId(), effectId);
        placeholders.put("balance", String.valueOf(plugin.getPlayerDataManager().getCoins(player.getUniqueId())));
